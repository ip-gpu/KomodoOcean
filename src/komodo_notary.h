...
int32_t komodo_notaries(uint8_t pubkeys[64][33],int32_t height)
{
    ...
    HASH_ITER(hh,Pubkeys[htind].Notaries,kp,tmp)
    {
        ...
        } else LogPrintf("illegal notaryid.%d vs n.%d\n",kp->notaryid,n);
    }
    ...
    if ( kp != 0 )
    {
        if ( (numnotaries= Pubkeys[htind].numnotaries) > 0 )
        {
            *notaryidp = kp->notaryid;
            ...
            //LogPrintf("found notary.%d ht.%d modval.%d\n",kp->notaryid,height,modval);
        } else LogPrintf("unexpected zero notaries at height.%d\n",height);
    } //else LogPrintf("cant find kp at htind.%d ht.%d\n",htind,height);
    //int32_t i; for (i=0; i<33; i++)
    //    LogPrintf("%02x",pubkey33[i]);
    //LogPrintf(" ht.%d notary.%d special.%d htind.%d num.%d\n",height,*notaryidp,modval,htind,numnotaries);
    return(modval);
}
...
void komodo_notarized_update(struct komodo_state *sp,int32_t nHeight,int32_t notarized_height,uint256 notarized_hash,uint256 notarized_desttxid)
{
    ...
    if ( 0 && ASSETCHAINS_SYMBOL[0] != 0 )
        LogPrintf("[%s] komodo_notarized_update nHeight.%d notarized_height.%d\n",ASSETCHAINS_SYMBOL,nHeight,notarized_height);
...
void komodo_init(int32_t height)
{
    ...
    else if ( 0 && height == KOMODO_MAINNET_START )
    {
        ...
        LogPrintf("set MAINNET notaries.%d\n",k);
    ...
    if ( (array= cJSON_Parse(jsonstr)) != 0 )
    {
        if ( (n= cJSON_GetArraySize(array)) > 0 )
        {
            for (i=0; i<n; i++)
            {
                ...
                {
                    LogPrintf("illegal hexstr.(%s) i.%d of n.%d\n",hexstr,i,n);
                    break;
                }
            }
            if ( i == n )
            ...
            } else LogPrintf("komodo_assetchain_pubkeys i.%d vs n.%d\n",i,n);
        } else LogPrintf("assetchain pubkeys n.%d\n",n);
    }
    //else if ( jsonstr != 0 )
    //    LogPrintf("assetchain pubkeys couldnt parse.(%s)\n",jsonstr);
}
