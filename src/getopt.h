      ...
      switch (c)
        ...
        case '9':
          if (digit_optind != 0 && digit_optind != this_option_optind)
            LogPrintf ("digits occur in two different argv-elements.\n");
        ...
        case 'c':
          LogPrintf ("option c with value `%s'\n", optarg);
          break;
        ...
        default:
          LogPrintf ("?? getopt returned character code 0%o ??\n", c);
        }
    }

  if (optind < argc)
    {
      LogPrintf ("non-option ARGV-elements: ");
      while (optind < argc)
        LogPrintf ("%s ", argv[optind++]);
      LogPrintf ("\n");
    }
