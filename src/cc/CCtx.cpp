...
std::string FinalizeCCTx(uint64_t CCmask,struct CCcontract_info *cp,CMutableTransaction &mtx,CPubKey mypk,uint64_t txfee,CScript opret)
{
    ...
    //LogPrintf("myCCaddr.(%s) %p vs unspendable.(%s) %p\n",myaddr,mycond,unspendable,othercond);
    ...
    for (i=0; i<n; i++)
    {
        ...
        } else LogPrintf("FinalizeCCTx couldnt find %s\n",mtx.vin[i].prevout.hash.ToString().c_str());
    }
    nmask = (1LL << n) - 1;
        ...
        if ( GetTransaction(mtx.vin[i].prevout.hash,vintx,hashBlock,false) != 0 )
        {
            ...
            else
            {
                ...
                else if ( strcmp(destaddr,unspendable) == 0 )
                {
                    ...
                    cond = othercond;
                    //LogPrintf("unspendable CC addr.(%s)\n",unspendable);
                }
                else if ( strcmp(destaddr,cp->unspendableaddr2) == 0)
                {
                    //LogPrintf("matched %s unspendable2!\n",cp->unspendableaddr2);
                ...
                {
                    //LogPrintf("matched %s unspendable3!\n",cp->unspendableaddr3);
                ...
                {
                    LogPrintf("CC signing error: vini.%d has unknown CC address.(%s)\n",i,destaddr);
                    continue;
                }
                ...
                if ( cc_signTreeSecp256k1Msg32(cond,privkey,sighash.begin()) != 0 )
                {
                    //int32_t z;
                    //for (z=0; z<32; z++)
                    //    LogPrintf("%02x",((uint8_t *)sighash.begin())[z]);
                    //LogPrintf(" sighash, ");
                    //for (z=0; z<32; z++)
                    //   LogPrintf("%02x",privkey[z]);
                    //LogPrintf(" signed with privkey\n");
                    mtx.vin[i].scriptSig = CCSig(cond);
                }
                else
                {
                    LogPrintf("vini.%d has CC signing error address.(%s)\n",i,destaddr);
                }
            }
        } else LogPrintf("FinalizeCCTx couldnt find %s\n",mtx.vin[i].prevout.hash.ToString().c_str());
    }     
...
void SetCCunspents(std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > &unspentOutputs,char *coinaddr)
{
    ...
    for (i=0; i<=n; i++)
        ptr[i] = coinaddr[i];
    CKomodoAddress address(addrstr);
...
int32_t CC_vinselect(int32_t *aboveip,int64_t *abovep,int32_t *belowip,int64_t *belowp,struct CC_utxo utxos[],int32_t numunspents,int64_t value)
{
    ...
    for (above=below=i=0; i<numunspents; i++)
    {
        ...
        //LogPrintf("value %.8f gap %.8f abovei.%d %.8f belowi.%d %.8f\n",dstr(value),dstr(gap),abovei,dstr(above),belowi,dstr(below));
    }
    *aboveip = abovei;
    *abovep = above;
    *belowip = belowi;
    *belowp = below;
    //LogPrintf("above.%d below.%d\n",abovei,belowi);
...
int64_t AddNormalinputs(CMutableTransaction &mtx,CPubKey mypk,int64_t total,int32_t maxinputs)
{
        ...
        if ( out.fSpendable != 0 && out.tx->vout[out.i].nValue >= threshold )
        {
            ...
            if ( myGetTransaction(txid,tx,hashBlock) != 0 && tx.vout.size() > 0 && vout < tx.vout.size() && tx.vout[vout].scriptPubKey.IsPayToCryptoCondition() == 0 )
            {
                ...
                if ( myIsutxo_spentinmempool(txid,vout) == 0 )
                {
                    ...
                    up->vout = vout;
                    sum += up->nValue;
                    //LogPrintf("add %.8f to vins array.%d of %d\n",(double)up->nValue/COIN,n,maxutxos);
    ...
    for (i=0; i<maxinputs && n>0; i++)
    {
        ...
        //LogPrintf("totalinputs %.8f vs total %.8f i.%d vs max.%d\n",(double)totalinputs/COIN,(double)total/COIN,i,maxinputs);
    ...
    {
        //LogPrintf("return totalinputs %.8f\n",(double)totalinputs/COIN);
        return(totalinputs);
    }
#endif
    return(0);
