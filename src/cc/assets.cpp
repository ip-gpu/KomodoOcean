...
bool AssetsValidate(struct CCcontract_info *cpAssets,Eval* eval,const CTransaction &tx, uint32_t nIn)
{
    ...
    if ( chainName.isSymbol("SEC") && chainActive.Height() <= 144073)
        return true;
    
    if ( chainName.isSymbol("MGNX") && chainActive.Height() <= 210190)
        return true;
    ...
    switch( funcid )
    {
        ...
        case 'b': // buyoffer
            ...
            preventCCvins = 1;
            preventCCvouts = 1;
            LogPrintf("buy offer validated to destaddr.(%s)\n",cpAssets->unspendableCCaddr);
            break;
        ...
        case 'o': // cancelbuy
            ...
            preventCCvins = 3;
            preventCCvouts = 0;
            //LogPrintf(stderr,"cancelbuy validated to origaddr.(%s)\n",origNormalAddr);
            break;
