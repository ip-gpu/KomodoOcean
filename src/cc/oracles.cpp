...
uint8_t DecodeOraclesCreateOpRet(const CScript &scriptPubKey,std::string &name,std::string &description,std::string &format)
{
    ...
    if ( script[0] == EVAL_ORACLES )
    {
        if ( script[1] == 'C' )
        {
            ...
            {
                return(script[1]);
            } else LogPrintf("DecodeOraclesCreateOpRet unmarshal error for C\n");
        }
    }
    return(0);
}
...
uint8_t DecodeOraclesData(const CScript &scriptPubKey,uint256 &oracletxid,uint256 &batontxid,CPubKey &pk,std::vector <uint8_t>&data)
{
    ...
    if ( vopret.size() > 1 && E_UNMARSHAL(vopret,ss >> e; ss >> f; ss >> oracletxid; ss >> batontxid; ss >> pk; ss >> data) != 0 )
    {
        if ( e == EVAL_ORACLES && f == 'D' )
            return(f);
        //else LogPrintf("DecodeOraclesData evalcode.%d f.%c\n",e,f);
    } //else LogPrintf("DecodeOraclesData not enough opereturn data\n");
    return(0);
}
...
CPubKey OracleBatonPk(char *batonaddr,struct CCcontract_info *cp)
{
    ...
    {
        for (i=0; i<32; i++)
            LogPrintf("%02x",cp->unspendablepriv2[i]);
        LogPrintf(" invalid privkey\n");
            ...
            if ( DecodeOraclesOpRet(tx.vout[numvouts-1].scriptPubKey,oracletxid,pk,dfee) == 'R' )
            {
                if ( oracletxid == reforacletxid && pk == publisher )
                {
                    if ( height > dheight || (height == dheight && dfee < datafee) )
                    {
                        ...
                        datafee = dfee;
                        if ( 0 && dheight != 0 )
                            LogPrintf("set datafee %.8f height.%d\n",(double)datafee/COIN,height);
                    }
                }
            }
        }
...
static uint256 myIs_baton_spentinmempool(uint256 batontxid,int32_t batonvout)
{
    BOOST_FOREACH(const CTxMemPoolEntry &e,mempool.mapTx)
    {
        ...
        if ( tx.vout.size() > 0 && tx.vin.size() > 1 && batontxid == tx.vin[1].prevout.hash && batonvout == tx.vin[1].prevout.n )
        {
            ...
            //char str[65]; LogPrintf("found baton spent in mempool %s\n",uint256_str(str,txid));
            return(txid);
        }
    }
    return(batontxid);
                ...
                if ( (ptr[1] == 'D' && DecodeOraclesData(tx.vout[numvouts-1].scriptPubKey,oracletxid,btxid,pk,data) == 'D') || (ptr[1] == 'R' && DecodeOraclesOpRet(tx.vout[numvouts-1].scriptPubKey,oracletxid,pk,dfee) == 'R') )
                {
                    if ( oracletxid == reforacletxid && pk == publisher )
                    {
                        if ( height > dheight )
                        {
                            ...
                            if ( ptr[1] == 'D' )
                                dataarg = data;
                            //char str[65]; LogPrintf("set batontxid %s height.%d\n",uint256_str(str,batontxid),height);
                        }
                    }
                }
            }
...
int64_t correlate_price(int32_t height,int64_t *prices,int32_t n)
{
    ...
    for (i=0; i<n; i++)
        LogPrintf("%llu ",(long long)prices[i]);
        ...
        if ( vopret.size() > 2 )
        {
            ...
            switch ( script[1] )
            {
                ...
                case 'D': // data
                    ...
                    break;
                default:
                    LogPrintf("illegal oracles funcid.(%c)\n",script[1]);
...
int64_t AddOracleInputs(struct CCcontract_info *cp,CMutableTransaction &mtx,uint256 oracletxid,CPubKey pk,int64_t total,int32_t maxinputs)
{
    ...
    //LogPrintf("addoracleinputs from (%s)\n",coinaddr);
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++)
    {
        txid = it->first.txhash;
        vout = (int32_t)it->first.index;
        //char str[65]; LogPrintf("oracle check %s/v%d\n",uint256_str(str,txid),vout);
        if ( GetTransaction(txid,vintx,hashBlock,false) != 0 && (numvouts=vintx.vout.size()-1)>0)
        {
            if ((funcid=DecodeOraclesOpRet(vintx.vout[numvouts].scriptPubKey,tmporacletxid,tmppk,tmpnum))!=0 && (funcid=='S' || funcid=='D'))
            {
                if (funcid=='D' && DecodeOraclesData(vintx.vout[numvouts].scriptPubKey,tmporacletxid,tmpbatontxid,tmppk,data)==0)
                    LogPrintf("invalid oraclesdata transaction \n");
                ...
                {  
                    ...
                    if ( (nValue= IsOraclesvout(cp,vintx,vout)) >= 10000 && myIsutxo_spentinmempool(txid,vout) == 0 )
                    {
                        ...
                        if ( (total > 0 && totalinputs >= total) || (maxinputs > 0 && n >= maxinputs) )
                            break;
                    } //else LogPrintf("nValue %.8f or utxo memspent\n",(double)nValue/COIN);
                }            
            }
        } else LogPrintf("couldnt find transaction\n");
    }
    return(totalinputs);
}
...
std::string OracleRegister(int64_t txfee,uint256 oracletxid,int64_t datafee)
{
    ...
    if ( datafee < txfee )
    {
        LogPrintf("datafee must be txfee or more\n");
        return("");
    }
    mypk = pubkey2pk(Mypubkey());
...
std::string OracleData(int64_t txfee,uint256 oracletxid,std::vector <uint8_t> data)
{
    ...
    if ( AddNormalinputs(mtx,mypk,2*txfee,3) > 0 ) // have enough funds even if baton utxo not there
    {
        ...
        else LogPrintf("warning: couldnt find baton utxo %s\n",batonaddr);
