...
int32_t _dicehash_clear(uint256 bettxid)
{
    ...
    if ( ptr != 0 )
    {
        ...
        return(0);
    } else LogPrintf("hashdelete couldnt find %s\n",bettxid.GetHex().c_str());
    return(-1);
}
...
int32_t DiceEntropyUsed(CTransaction &oldbetTx,uint256 &oldbettxid,int32_t &oldentropyvout,uint256 entropyused,uint256 bettxid,CTransaction betTx,int32_t entropyvout)
{
    ...
    {
        LogPrintf("null entropyused or bettxid\n");
        return(1);
    }
            ...
            if ( funcid == 'R' || (retval= DiceEntropyUsed(oldbetTx,oldbettxid,oldentropyvout,entropyused,bettxid,betTx,entropyvout)) >= 0 )
            {
                ...
                if ( myAddtomempool(tx) != 0 )
                {
                    ...
                    if ( retval == 0 )
                    {
                        ...
                        pthread_mutex_unlock(&DICEREVEALED_MUTEX);
                        LogPrintf("added.%c to mempool.[%d] and broadcast entropyused.%s bettxid.%s -> %s\n",funcid,i,entropyused.GetHex().c_str(),bettxid.GetHex().c_str(),tx.GetHash().GetHex().c_str());
                    } else LogPrintf("rebroadcast.%c to mempool.[%d] and broadcast entropyused.%s bettxid.%s -> %s\n",funcid,i,entropyused.GetHex().c_str(),bettxid.GetHex().c_str(),tx.GetHash().GetHex().c_str());
                    return(true);
                }
                else
                {
                    RelayTransaction(tx);
                    LogPrintf("rebroadcast.%c and clear [%d] and broadcast entropyused.%s bettxid.%s -> %s\n",funcid,i,entropyused.GetHex().c_str(),bettxid.GetHex().c_str(),tx.GetHash().GetHex().c_str());
                    if ( ptr != 0 )
                    {
                    ...
                    //LogPrintf("error adding funcid.%c E.%s bet.%s -> %s to mempool, probably Disable replacement feature size.%d\n",funcid,entropyused.GetHex().c_str(),bettxid.GetHex().c_str(),tx.GetHash().GetHex().c_str(),(int32_t)ptr->rawtx.size());
                }
            } else LogPrintf("error duplicate entropyused different bettxid\n");
        } else LogPrintf("error decoding hex\n");
    }
    return(false);
}
...
int32_t dice_betspent(char *debugstr,uint256 bettxid)
{
    ...
    {
        //LogPrintf("%s txid.%s already spent\n",debugstr,bettxid.GetHex().c_str());
        return(1);
    }
    {
        //LOCK(mempool.cs);
        ...
        {
            LogPrintf("%s bettxid.%s already spent in mempool\n",debugstr,bettxid.GetHex().c_str());
            return(-1);
        }
    }
    return(0);
...
void *dicefinish(void *_ptr)
{
    ...
    while ( 1 )
    {
        if ( newht != 0 && lastheight != newht )
        {
            lastheight = newht;
            newblock = 1;
            LogPrintf("dicefinish process lastheight.%d <- newht.%d\n",lastheight,newht);
        } else newblock = 0;
        ...
        for (iter=-1; iter<=1; iter+=2)
        {
            ...
            DL_FOREACH_SAFE(DICEFINISH_LIST,ptr,tmp)
            {
                ...
                else if ( newblock != 0 && ptr->txid != zeroid )
                {
                    ...
                    {
                        ptr->orphaned++;
                        LogPrintf("ORPHANED.%d finish txid.%s\n",ptr->orphaned,ptr->txid.GetHex().c_str());
                        if ( ptr->orphaned < 4 )
                            continue;
                        if ( ptr->rawtx.empty() == 0 )
                            ptr->rawtx.clear();
                ...
                if ( ptr->bettxid_ready != 0 )
                {
                    ...
                    {
                        LogPrintf("purge bettxid_ready %s\n",ptr->bettxid.GetHex().c_str());
                        dicefinish_delete(ptr);
                        continue;
                    }
                    else if ( newblock != 0 )
                    {
                        if ( ptr->txid != zeroid )
                        {
                            ...
                            //LogPrintf("duration finish txid.%s\n",ptr->txid.GetHex().c_str());
                            if ( numblocks == 0 )
                                mySenddicetransaction(ptr->rawtx,ptr->entropyused,ptr->entropyvout,ptr->bettxid,ptr->betTx,ptr->funcid,ptr);
                            else continue;
                        }
...
//LogPrintf("iter.%d vin0_needed.%d got %d, num 0.0002 %d\n",iter,vin0_needed,n,num);
                    ...
                    LogPrintf("error issing cc/dapps/sendmany100\n");
                free(utxos);
            }
        }
...
void DiceQueue(int32_t iswin,uint64_t sbits,uint256 fundingtxid,uint256 bettxid,CTransaction betTx,int32_t entropyvout)
{
    ...
    if ( didinit == 0 )
    {
        ...
        {
            LogPrintf("error launching dicefinish thread\n");
            return;
        }
    }
...
CPubKey DiceFundingPk(CScript scriptPubKey)
{
    ...
    } else LogPrintf("DiceFundingPk invalid size.%d\n",(int32_t)scriptPubKey.size());
    return(pk);
}
...
uint256 DiceHashEntropy(uint256 &entropy,uint256 _txidpriv,int32_t vout,int32_t usevout)
{
    ...
    {
        for (i=0; i<32; i++)
            LogPrintf("%02x",ssecret.bytes[i]);
        LogPrintf(" ssecret\n");
        for (i=0; i<32; i++)
            LogPrintf("%02x",ssecret2.bytes[i]);
        LogPrintf(" ssecret2 dont match\n");
    }
    memset(tmp256.bytes,0,32);
    //char str[65],str2[65];
    //LogPrintf("generated house hentropy.%s <- entropy.%s\n",uint256_str(str,hentropy),uint256_str(str2,entropy));
    return(hentropy);
}
...
uint64_t DiceCalc(int64_t bet,int64_t vout2,int64_t minbet,int64_t maxbet,int64_t maxodds,int64_t timeoutblocks,uint256 houseentropy,uint256 bettorentropy)
{
    ...
    if ( vout2 <= 10000 )
    {
        LogPrintf("unexpected vout2.%llu\n",(long long)vout2);
        return(0);
    }
...
uint8_t DecodeDiceOpRet(uint256 txid,const CScript &scriptPubKey,uint64_t &sbits,uint256 &fundingtxid,uint256 &hash,uint256 &proof)
{
    ...
    if ( vopret.size() > 2 )//&& script[0] == 0x6a )
    {
        ...
        } else LogPrintf("script[0] %02x != EVAL_DICE\n",script[0]);
...
bool DiceValidate(struct CCcontract_info *cp,Eval *eval,const CTransaction &tx, uint32_t nIn)
{
    ...
    else
    {
        ...
        if ( (funcid=  DecodeDiceOpRet(txid,tx.vout[numvouts-1].scriptPubKey,refsbits,fundingtxid,hash,proof)) != 0 )
        {
            ...
            if ( sbits != refsbits )
            {
                LogPrintf("VALIDATION ERROR: sbits %llx != refsbits %llx\n",(long long)sbits,(long long)refsbits);
            ...
            switch ( funcid )
            {
                ...
                case 'B':
                    ...
                    {
                        //LogPrintf("why does node1 get VALIDATION ERROR: invalid dicebet bettxid %s\n",txid.GetHex().c_str());
                ...
                case 'T':
                    ...
                    if ( iswin != 0 )
                    {
                        ...
                        //LogPrintf("calculated house hentropy.%s\n",uint256_str(str,hash));
                        //LogPrintf("verify house entropy %s vs bettor %s\n",uint256_str(str,proof),uint256_str(str2,entropy));
                ...
                case 'R':
                    ...
                    else if ( vinTx.vout[tx.vin[0].prevout.n].scriptPubKey != fundingPubKey )
                    {
                        ...
                        {
                            LogPrintf("cmpaddr.%s != fundingaddr.%s\n",cmpaddr,fundingaddr);
                            return eval->Invalid("vin.0 not from fundingPubKey for refund");
                        }
                    }
                    if ( (rand() % 1000) == 0 )
                        LogPrintf("add more validation for refunds\n");
                    break;
                default:
                    LogPrintf("illegal dice funcid.(%c)\n",funcid);
                        ...
                        if ( first == 0 && (funcid == 'E' || funcid == 'W' || funcid == 'L') )
                        {
                            ...
                            if ( tx.vout.size() > 1 && fundingPubKey == tx.vout[1].scriptPubKey )
                            {
                                ...
                                if ( myIsutxo_spentinmempool(ignoretxid,ignorevin,txid,vout) == 0 )
                                {
                                    ...
                                    //LogPrintf("funcid.%c first.%d entropytxid.%s val %.8f\n",funcid,first,txid.GetHex().c_str(),(double)entropyval/COIN);
                                    first = 1;
                                    if (random) {
                                        LogPrintf( "chosen entropy on loop: %d\n",loops);
                                    }
                                }
                            }
                            else
    ...
    if ( fundingtxid != zeroid ) // avoid scan unless creating new funding plan
    {
        ...
        if ( myGetTransaction(fundingtxid,tx,hashBlock) != 0 && tx.vout.size() > 1 && ConstrainVout(tx.vout[0],1,CCaddr,0) != 0 )
        {
            if ( DecodeDiceFundingOpRet(tx.vout[tx.vout.size()-1].scriptPubKey,sbits,minbet,maxbet,maxodds,timeoutblocks) == 'F' && sbits == refsbits )
            {
                fundingPubKey = tx.vout[1].scriptPubKey;
                return(true);
            } else LogPrintf("error decoding opret or sbits mismatch %llx vs %llx\n",(long long)sbits,(long long)refsbits);
        } else LogPrintf("couldnt get funding tx\n");
        return(false);
    }
...
struct CCcontract_info *Diceinit(CScript &fundingPubKey,uint256 reffundingtxid,struct CCcontract_info *C,char *planstr,uint64_t &txfee,CPubKey &mypk,CPubKey &dicepk,uint64_t &sbits,int64_t &minbet,int64_t &maxbet,int64_t &maxodds,int64_t &timeoutblocks)
{
    ...
    {
        LogPrintf("Dice plan (%s) exists.%d vs cmpflag.%d\n",planstr,!cmpflag,cmpflag);
        return(0);
    }
    return(cp);
}
...
UniValue DiceInfo(uint256 diceid)
{
    ...
    {
        LogPrintf("fundingtxid isnt dice creation txid\n");
...
std::string DiceBetFinish(uint8_t &funcid,uint256 &entropyused,int32_t &entropyvout,int32_t *resultp,uint64_t txfee,char *planstr,uint256 fundingtxid,uint256 bettxid,int32_t winlosetimeout,uint256 vin0txid,int32_t vin0vout)
{
    ...
    *resultp = 0;
    ...
    if ( winlosetimeout != 0 ) // must be dealernode
    {
        if ( scriptPubKey != fundingPubKey )
        {
            //LogPrintf("only dice fund creator can submit winner or loser\n");
            winlosetimeout = 0;
        }
    }
    if ( myGetTransaction(bettxid,betTx,hashBlock) != 0 && myGetTransaction(betTx.vin[0].prevout.hash,entropyTx,hashBlock) != 0 )
    {
        ...
        if ( winlosetimeout == 0 || (iswin= DiceIsWinner(hentropyproof,entropyvout,bettxid,betTx,entropyTx,bettorentropy,sbits,minbet,maxbet,maxodds,timeoutblocks,fundingtxid)) != 0 )
        {
            ...
            if ( winlosetimeout != 0 ) // dealernode
            {
                ...
                //LogPrintf("set winlosetimeout %d <- %d\n",winlosetimeout,iswin);
...
void *dealer0_loop(void *_arg)
{
    ...
    {
        LogPrintf("error initializing dealer0_loop\n");
        StartShutdown();
    }
    LogPrintf("dealer0 node running\n");
    height = lastht = 0;
    ...
    while ( 1 )
    {
        ...
        LogPrintf("have %d 0.0002 utxos, need %d\n",num,DICE_MINUTXOS);
        if ( num < DICE_MINUTXOS ) // this deadlocks, need to put it in a different thread
        {
            char *cmd = (char *)malloc(100 * 128);
            sprintf(cmd,"./komodo-cli -ac_name=%s sendmany \"\"  \"{\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002,\\\"%s\\\":0.0002}\"",chainName.symbol().c_str(),coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr,coinaddr);
            n = sqrt((DICE_MINUTXOS - num) / 100)*2 + 1;
            LogPrintf("num normal 0.0002 utxos.%d < %d -> n.%d\n",num,DICE_MINUTXOS,n);
            for (i=0; i<n; i++)
            {
                LogPrintf("%d of %d: ",i,n);
                if ( system(cmd) != 0 )
                    LogPrintf("system error issuing.(%s)\n",cmd);
            }
            free(cmd);
            if ( (rand() % 100) == 0 )
            {
                LogPrintf("make 0.023 utxos\n");
                if ( system("cc/dapps/sendmany") != 0 )
                    LogPrintf("system error issuing.(cc/dapps/sendmany)\n");
            }
        }
#ifdef WIN32
