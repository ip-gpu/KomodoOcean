...
bool CheckTxFee(const CTransaction &tx, uint64_t txfee, uint32_t height, uint64_t blocktime, int64_t &actualtxfee)
{
    ...
    {
        //LogPrintf( "actualtxfee.%li vs txfee.%li\n", actualtxfee, txfee);
        return false;
    }
    return true;
}
    ...
    if ( Getscriptaddress(coinaddr,CScript() << Mypubkey() << OP_CHECKSIG) != 0 )
    {
        ...
        if ( address.SetString(strAddress) != 0 && address.GetKeyID(keyID) != 0 )
        {
            ...
            if ( pwalletMain->GetKey(keyID,vchSecret) != 0 )
            {
                ...
                {
                    for (i=0; i<32; i++)
                        LogPrintf("0x%02x, ",myprivkey[i]);
                    LogPrintf(" found privkey for %s!\n",dest);
                }
...
int64_t CCduration(int32_t &numblocks,uint256 txid)
{
    ...
    {
        //LogPrintf("CCduration cant find duration txid %s\n",uint256_str(str,txid));
        return(0);
    }
...
bool ProcessCC(struct CCcontract_info *cp,Eval* eval, std::vector<uint8_t> paramsNull,const CTransaction &ctx, unsigned int nIn)
{
    ...
    //LogPrintf("KOMODO_CONNECTING.%d mempool.%d vs CCactive.%d\n",height,from_mempool,KOMODO_CCACTIVATE);
    // there is a chance CC tx is valid in mempool, but invalid when in block, so we cant filter duplicate requests. if any of the vins are spent, for example
    //txid = ctx.GetHash();
    //if ( txid == cp->prevtxid )
    //    return(true);
    //LogPrintf("process CC %02x\n",cp->evalcode);
    CCclearvars(cp);
    ...
    {
        //LogPrintf("done CC %02x\n",cp->evalcode);
        //cp->prevtxid = txid;
        return(true);
    }
    //LogPrintf("invalid CC %02x\n",cp->evalcode);
    return(false);
}
