...
uint8_t DecodeChannelsOpRet(const CScript &scriptPubKey,uint256 &opentxid, CPubKey &srcpub,CPubKey &destpub,int32_t &numpayments,int64_t &payment,uint256 &hashchain)
{
    ...
    if ( vopret.size() > 2 )
    {
        ...
        } else LogPrintf("script[0] %02x != EVAL_CHANNELS\n",script[0]);
...
bool ChannelsValidate(struct CCcontract_info *cp,Eval* eval,const CTransaction &tx, uint32_t nIn)
{
    ...
    else
    {
        ...
        {
            LogPrintf("Channelsget invalid amount\n");
            return false;
        }
        else
        {
            ...
            if ( (funcid = DecodeChannelsOpRet(tx.vout[numvouts-1].scriptPubKey, opentxid, srcpub, destpub, param1, param2, param3)) != 0)
            {
                switch ( funcid )
                {
                    ...
                    case 'R':
                        ...
                        break;
                    default:
                        LogPrintf("illegal channels funcid.(%c)\n",funcid);
            ...
            if ( retval != 0 )
                LogPrintf("Channel tx validated\n");
            else LogPrintf("Channel tx invalid\n");
            return(retval);
        }
    }
}
...
int64_t AddChannelsInputs(struct CCcontract_info *cp,CMutableTransaction &mtx, CTransaction openTx, uint256 &prevtxid)
{
    ...
    {
        LogPrintf("invalid channel open txid\n");
        return 0;
    }
    ...
    if (AddNormalinputs(mtx,mypk,2*txfee,3) > 0)
    {
        if ((funds=AddChannelsInputs(cp,mtx,channelOpenTx,prevtxid)) !=0 && (change=funds-amount-txfee)>=0)
        {
            if ((numvouts=channelOpenTx.vout.size()) > 0 && DecodeChannelsOpRet(channelOpenTx.vout[numvouts-1].scriptPubKey, txid, srcpub, destpub, totalnumpayments, payment, hashchain)=='O')
            {
                ...
                {
                    LogPrintf("this is not our channel\n");
                    return("");
                }
                else if (amount % payment != 0 || amount<payment)
                {
                    LogPrintf("invalid amount, not a magnitude of payment size\n");
                    return ("");
                }
                  ...
                  (funcid == 'P' || funcid=='O'))
                {
                    ...
                    {
                        LogPrintf("not enough funds in channel for that amount\n");
                        return ("");
                    ...
                    if (secret!=zeroid)
                    {
                        ...
                        {
                            LogPrintf("invalid secret supplied\n");
                            return("");
                        }
                    }
                    else
                ...
                {
                    LogPrintf("invalid previous tx\n");
                    return("");
                }
            }
            else
            {
                LogPrintf( "invalid channel open tx\n");
                return ("");
            }
        ...
        {
            LogPrintf("error adding CC inputs\n");
            return("");
        }
    }
    LogPrintf("error adding normal inputs\n");
    return("");
}
...
std::string ChannelClose(uint64_t txfee,uint256 opentxid)
{
    ...
    if (mypk != srcpub)
    {
        LogPrintf("cannot close, you are not channel owner\n");
        return("");
    }
    if ( AddNormalinputs(mtx,mypk,2*txfee,3) > 0 )
    {
...
std::string ChannelRefund(uint64_t txfee,uint256 opentxid,uint256 closetxid)
{
    ...
    {
        LogPrintf( "invalid channel close txid\n");
        return ("");
    }
    ...
    {
        LogPrintf( "invalid channel close tx\n");
        return ("");
    }
    if (txid!=opentxid)
    {
        LogPrintf( "open and close txid are not from same channel\n");
        return ("");
    }
    if (GetTransaction(opentxid,channelOpenTx,hashblock,false) == 0)
    {
    ...
    if (mypk != srcpub)
    {
        LogPrintf("cannot refund, you are not the channel owenr\n");
        return("");
    }
    if ( AddNormalinputs(mtx,mypk,2*txfee,3) > 0 )
    {
        if ((funds=AddChannelsInputs(cp,mtx,channelOpenTx,prevtxid)) !=0 && funds-txfee>0)
        {
            ...
            {
                LogPrintf("previous tx is invalid\n");
                return("");
            }
        }
        else
