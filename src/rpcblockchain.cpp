...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in coins\n"
...
UniValue passphrasewif(const UniValue& params, bool fHelp)
{
    ...
    ctx = komodo_ctx();
    komodo_priv2pub(ctx,pubkey33,coinaddr,privkey,0,60);
    komodo_priv2wif(0,wifstr,privkey,188);
    free(ctx);
    ...
    if ( (n= komodo_minerids(minerids,height,(int32_t)(sizeof(minerids)/sizeof(*minerids)))) > 0 )
    {
        ...
        if ( numnotaries > 0 )
        {
            ...
            for (i=0; i<64; i++)
            {
                ...
                komodo_address(kmdaddr,60,pubkeys[i],33);
...
UniValue notaries(const UniValue& params, bool fHelp)
{
    ...
    if ( height < 0 )
        height = chainActive.Tip()->nHeight;
    //LogPrintf("notaries as of height.%d\n",height);
    ...
    //else
    {
        if ( (n= komodo_notaries(pubkeys,height)) > 0 )
        {
            for (i=0; i<n; i++)
            {
                ...
                komodo_address(btcaddr,0,pubkeys[i],33);
