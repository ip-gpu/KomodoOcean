...
#define MAX_NUMBER_BLOCK_CACHED_FILES 256
...
bool CBlockTreeDB::LoadBlockIndexGuts()
{
    ...
    // Check real number of files in future
    // Now set it to 256
    FILE *file_cached[MAX_NUMBER_BLOCK_CACHED_FILES];
    CBufferedFile *buffered_file[MAX_NUMBER_BLOCK_CACHED_FILES];

    for (int i=0; i<MAX_NUMBER_BLOCK_CACHED_FILES; i++)
    {
        file_cached[i] = NULL;
        buffered_file[i] = NULL;
    }
    ...
    for (int i=0; i<MAX_NUMBER_BLOCK_CACHED_FILES; i++)
    {
        ...
        if (!stat(GetBlockPosFilename(pos, "blk").string().c_str(), &stat_buffer))
        {
            file_cached[i] = OpenBlockFile(pos, true);
            if (file_cached[i])
            {
                fseek(file_cached[i],0,SEEK_END);
                int32_t fsize = (int32_t)ftell(file_cached[i]);
                rewind(file_cached[i]);

                buffered_file[i] = new CBufferedFile(file_cached[i], fsize, 0, SER_DISK, CLIENT_VERSION);
                buffered_file[i]->Fill();
            }
        }
    }

    LogPrintf("Prebuffered blocks.\n");
    ...
    while (pcursor->Valid()) {
        ...
        try {
            ...
            if (chType == DB_BLOCK_INDEX) {
                ...
                if ((diskindex.nFile < MAX_NUMBER_BLOCK_CACHED_FILES) && buffered_file[diskindex.nFile])
                     komodo_index2pubkey33(pubkey33,pindexNew,pindexNew->nHeight,buffered_file[diskindex.nFile]);
                else
