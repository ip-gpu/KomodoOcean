...
static bool ActivateBestChainStep(CValidationState &state, CBlockIndex *pindexMostWork, CBlock *pblock) {
    ...
    if ( KOMODO_REWIND != 0 )
    {
        ...
        LogPrintf("reached rewind.%d, best to do: ./komodo-cli stop\n",KOMODO_REWIND);
#ifndef _WIN32
        sleep(60);
#else
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (miSelf != mapBlockIndex.end()) {
        ...
        if ( pindex != 0 && IsInitialBlockDownload() == 0 ) // jl777
        {
            ...
            {
                pindex->nStatus |= BLOCK_FAILED_MASK;
                LogPrintf("known block failing CheckBlockHeader %d\n",(int32_t)pindex->nHeight);
                return false;
            }
            ...
            if (hash != chainparams.GetConsensus().hashGenesisBlock)
            {
                ...
                {
                    pindex->nStatus |= BLOCK_FAILED_MASK;
                    LogPrintf("known block.%d failing to find prevblock\n",(int32_t)pindex->nHeight);
                ...
                {
                    pindex->nStatus |= BLOCK_FAILED_MASK;
                    LogPrintf("known block.%d found invalid prevblock\n",(int32_t)pindex->nHeight);
            ...
            {
                pindex->nStatus |= BLOCK_FAILED_MASK;
                //LogPrintf("known block.%d failing ContextualCheckBlockHeader\n",(int32_t)pindex->nHeight);
                return false;
            }
        }
       return true;
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, bool fRequested, CDiskBlockPos* dbp)
{
    ...
    if ( pindex == 0 )
    {
        LogPrintf("AcceptBlock error null pindex\n");
        return false;
    }
