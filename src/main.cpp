    ...
    // Check for conflicts with in-memory transactions
    {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (pool.mapNextTx.count(outpoint))
            {
                ...
                //if ( counter++ < 100 )
                LogPrintf("Disable replacement feature for now\n");
                return false;
            }
        }
        BOOST_FOREACH(const JSDescription &joinsplit, tx.vjoinsplit)
...
bool IsInitialBlockDownload()
{
    ...
    {
        //LogPrintf("IsInitialBlockDownload: checkpoint -> initialdownload\n");
        return true;
    }
    static bool lockIBDState = false;
    if (lockIBDState)
    {
        //LogPrintf("lockIBDState true %d < %d\n",chainActive.Height(),pindexBestHeader->nHeight - 10);
        return false;
    }
    ...
    //LogPrintf("state.%d  ht.%d vs %d, t.%u %u\n",state,(int32_t)chainActive.Height(),(uint32_t)ptr->nHeight,(int32_t)ptr->GetBlockTime(),(uint32_t)(GetTime() - chainParams.MaxTipAge()));
    if (!state)
    {
        lockIBDState = true;
    }
...
//LogPrintf("ht.%d vout0 t%u\n",pindex->nHeight,tx.nLockTime);
    ...
    if ( ASSETCHAINS_OVERRIDE_PUBKEY33[0] != 0 && ASSETCHAINS_COMMISSION != 0 && block.vtx[0].vout.size() > 1 )
    {
        ...
        if ( (checktoshis = komodo_commission(block)) != 0 )
        {
            ...
            else LogPrintf("checktoshis %.8f vs actual vout[1] %.8f\n",dstr(checktoshis),dstr(block.vtx[0].vout[1].nValue));
        }
    }
                ...
                bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    {
        //LogPrintf("CheckBlockHeader error in CheckBlock fCheckPOW.%d\n",fCheckPOW);
        return false;
    }
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    {
        LogPrintf("CheckBlockHeader failed\n");
        return false;
    }
    ...
    if (pindex == NULL)
    {
        if ( (pindex= AddToBlockIndex(block)) == 0 )
            LogPrintf("couldnt add to block index\n");
    }
    if (ppindex)
        *ppindex = pindex;
    return true;
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, bool fRequested, CDiskBlockPos* dbp)
{
    ...
    {
        //LogPrintf("AcceptBlockHeader rejected\n");
        return false;
    }
    if ( pindex == 0 )
    {
        //LogPrintf("unexpected AcceptBlock error null pindex\n");
        return false;
    }
    ...
    if ((!CheckBlock(pindex->nHeight,pindex,block, state, verifier,0)) || !ContextualCheckBlock(block, state, pindex->pprev))
    {
        ...
        LogPrintf("CheckBlock or ContextualCheckBlock failed\n");
        return false;
    }
...
bool static LoadBlockIndexDB()
{
    ...
    if (!pblocktree->LoadBlockIndexGuts())
        return false;
    LogPrintf("%s: loaded block index guts...\n", __func__);
...
std::string GetWarnings(const std::string& strFor)
{
    ...
    // Alerts
    if ((strFor == "statusbar")||(strFor == "rpc"))
    {
        LOCK(cs_mapAlerts);
