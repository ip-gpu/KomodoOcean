...
int main(int argc, char * argv[])
{
    ...
    if ( argc < 2 )
    {
        ...
        LogPrintf("%s name supply endsubsidy reward halving decay\n",argv[0]);
        LogPrintf("%s -gen num name supply endsubsidy reward halving decay\n",argv[0]);
        return(-1);
    }
        ...
        for (i=0; i<num; i++)
        {
            ...
            while ( 1 )
            {
                ...
                if ( allocated[rpcport] == 0 && allocated[rpcport-1] == 0 )
                {
                    ...
                    {
                        LogPrintf("{ \"assetname\": \"%s\", \"p2p\": %u, \"rpc\": %u, \"supply\": %llu, \"ac_cc\": %u, \"ac_reward\": %llu, \"notarize\": %s }%s ",newname,rpcport-1,rpcport,(long long)supply+j,cc,(long long)reward,i<64?"true":"false",i<num-1?",":"");
                    }
                    allocated[rpcport] = 1;
                    allocated[rpcport-1] = 1;
                    break;
