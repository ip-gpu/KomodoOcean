...
extern struct CCType CC_Secp256k1Type;
...
static unsigned char *secp256k1Fingerprint(const CC *cond) {
    Secp256k1FingerprintContents_t *fp = (Secp256k1FingerprintContents_t *)calloc(1, sizeof(Secp256k1FingerprintContents_t));
    OCTET_STRING_fromBuf(&fp->publicKey, (const char*)(cond->publicKey), SECP256K1_PK_SIZE);
...
int cc_secp256k1VerifyTreeMsg32(const CC *cond, const unsigned char *msg32) {
    ...
    int out = cc_visit((CC*)cond, visitor);
    return out;
}
...
static int secp256k1Sign(CC *cond, CCVisitor visitor) {
    ...
    if (!cond->signature) cond->signature = (uint8_t*)calloc(1, SECP256K1_SIG_SIZE);
...
int cc_signTreeSecp256k1Msg32(CC *cond, const unsigned char *privateKey, const unsigned char *msg32) {
    ...
    // serialize pubkey
    unsigned char *publicKey = (unsigned char *)calloc(1, SECP256K1_PK_SIZE);
...
static CC *cc_secp256k1Condition(const unsigned char *publicKey, const unsigned char *signature) {
    ...
    pk = (unsigned char *)calloc(1, SECP256K1_PK_SIZE);
    memcpy(pk, publicKey, SECP256K1_PK_SIZE);
    if (signature) {
        sig = (unsigned char *)calloc(1, SECP256K1_SIG_SIZE);
...
static CC *secp256k1FromJSON(const cJSON *params, char *err) {
    ...
    if (!jsonGetHex(params, (char*)"publicKey", err, &pk, &pkSize)) goto END;

    if (!jsonGetHexOptional(params, (char*)"signature", err, &sig, &sigSize)) goto END;
...
static void secp256k1ToJSON(const CC *cond, cJSON *params) {
    jsonAddHex(params, (char*)"publicKey", cond->publicKey, SECP256K1_PK_SIZE);
    if (cond->signature) {
        jsonAddHex(params, (char*)"signature", cond->signature, SECP256K1_SIG_SIZE);
    }
}
...
static Fulfillment_t *secp256k1ToFulfillment(const CC *cond) {
    ...
    OCTET_STRING_fromBuf(&sec->publicKey, (const char*)(cond->publicKey), SECP256K1_PK_SIZE);
    OCTET_STRING_fromBuf(&sec->signature, (const char*)(cond->signature), SECP256K1_SIG_SIZE);
    return ffill;
}
