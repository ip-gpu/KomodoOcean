...
void appendUriSubtypes(uint32_t mask, unsigned char *buf) {
    ...
    for (int i=0; i<32; i++) {
        if (mask & 1 << i) {
            if (append) {
                strcat((char*)buf, ",");
                strcat((char*)buf, CCTypeRegistry[i]->name);
            } else {
                strcat((char*)buf, "&subtypes=");
                strcat((char*)buf, CCTypeRegistry[i]->name);
                append = 1;
            }
        }
...
char *cc_conditionUri(const CC *cond) {
    ...
    unsigned char *out = (unsigned char *)calloc(1, 1000);
    sprintf((char *)out, "ni:///sha-256;%s?fpt=%s&cost=%lu",
...
ConditionTypes_t asnSubtypes(uint32_t mask) {
    ...
    types.buf = (uint8_t*)calloc(1, types.size);
...
size_t cc_conditionBinary(const CC *cond, unsigned char *buf) {
    Condition_t *asn = (Condition_t *)calloc(1, sizeof(Condition_t));
...
enum CCTypeId cc_typeId(const CC *cond) {
    return (CCTypeId)(cc_isAnon(cond) ? cond->conditionType->typeId : cond->type->typeId);
}
