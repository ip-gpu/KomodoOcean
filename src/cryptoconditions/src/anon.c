...
CC *mkAnon(const Condition_t *asnCond) {
    ...
    if (!realType) {
        LogPrintf("Unknown ASN type: %i", asnCond->present);
        return 0;
    }
...
static void anonToJSON(const CC *cond, cJSON *params) {
    ...
    cJSON_AddItemToObject(params, "fingerprint", cJSON_CreateString((const char*)b64));
    free(b64);
...
static unsigned char *anonFingerprint(const CC *cond) {
    unsigned char *out = (unsigned char *)calloc(1, 32);
