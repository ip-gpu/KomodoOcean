...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    return CKomodoAddress(keyID).ToString();
}


CKomodoAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CKomodoAddress(account.vchPubKey.GetID());
}
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const PAIRTYPE(CKomodoAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        const CKomodoAddress& address = item.first;
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        BOOST_FOREACH(CTxDestination address, grouping)
        {
            ...
            addressInfo.push_back(CKomodoAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwalletMain->mapAddressBook.find(CKomodoAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CKomodoAddress(address).Get())->second.name);
            }
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    ...
    CKomodoAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    // Komodo address
    CKomodoAddress address = CKomodoAddress(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Komodo address");
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    string strAccount = AccountFromValue(params[0]);
    CKomodoAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Komodo address");
    CAmount nAmount = AmountFromValue(params[2]);
    if (nAmount <= 0)
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    ...
    return CKomodoAddress(innerID).ToString();
}
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    map<CKomodoAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CKomodoAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CKomodoAddress, tallyitem>::iterator it = mapTally.find(address);
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total confirmed zcash balance of the wallet in " + CURRENCY_UNIT + "\n"
            "  \"unconfirmed_balance\": xxx, (numeric) the total unconfirmed zcash balance of the wallet in " + CURRENCY_UNIT + "\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (size_t idx = 0; idx < inputs.size(); idx++) {
            ...
            CKomodoAddress address(input.get_str());
            if (!address.IsValid())
...
UniValue z_getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    pwalletMain->SetZAddressBook(pubaddr.Get(), strAccount, "receive");
...
CAmount getBalanceTaddr(std::string transparentAddress, int minDepth=1, bool ignoreUnspendable=true) {
    ...
    if (transparentAddress.length() > 0) {
        CKomodoAddress taddr = CKomodoAddress(transparentAddress);
        if (!taddr.IsValid()) {
...
UniValue z_getbalance(const UniValue& params, bool fHelp)
{
    ...
    CKomodoAddress taddr(fromaddress);
...
UniValue z_shieldcoinbase(const UniValue& params, bool fHelp)
{
    ...
    CKomodoAddress taddr;
    if (!isFromWildcard) {
        taddr = CKomodoAddress(fromaddress);
        if (!taddr.IsValid()) {
...
UniValue z_mergetoaddress(const UniValue& params, bool fHelp)
{
    ...
    CKomodoAddress taddr(destaddress);
    if (!taddr.IsValid()) {
        try {
...
{
    set<CKomodoAddress> setAddress;  int32_t i,siglen=0,nMinDepth = 1,nMaxDepth = 9999999; vector<COutput> vecOutputs; uint32_t eligible,earliest = 0; CScript best_scriptPubKey; arith_uint256 bnTarget; bool fNegative,fOverflow;
    ...
    *utxovaluep = 0;
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        {
            eligible = komodo_stake(0,bnTarget,(uint32_t)tipindex->nHeight+1,out.tx->GetHash(),out.i,*blocktimep,(uint32_t)tipindex->nTime,(char *)CKomodoAddress(address).ToString().c_str());
            if ( eligible > 0 )
            {
                ...
                else if ( earliest == 0 || eligible < earliest || (eligible == earliest && (*utxovaluep == 0 || nValue < *utxovaluep)) )
                {
                    ...
                    *txtimep = (uint32_t)out.tx->nLockTime;
                    LogPrintf("earliest.%u [%d] (%s) nValue %.8f locktime.%u\n",earliest,(int32_t)(earliest- *blocktimep),CKomodoAddress(address).ToString().c_str(),(double)nValue/COIN,*txtimep);
                }
            }
        }
    }
