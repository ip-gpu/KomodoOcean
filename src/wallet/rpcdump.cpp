...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    CKeyID vchAddress = pubkey.GetID();
    {
        ...
        if (pwalletMain->HaveKey(vchAddress)) {
            return CKomodoAddress(vchAddress).ToString();
        }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    return CKomodoSecret(vchSecret).ToString();
}
...
UniValue dumpwallet_impl(const UniValue& params, bool fHelp, bool fDumpZKeys)
{
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CKomodoAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            if (pwalletMain->mapAddressBook.count(keyid)) {
                file << strprintf("%s %s label=%s # addr=%s\n", CKomodoSecret(key).ToString(), strTime, EncodeDumpString(pwalletMain->mapAddressBook[keyid].name), strAddr);
            } else if (setKeyPool.count(keyid)) {
                file << strprintf("%s %s reserve=1 # addr=%s\n", CKomodoSecret(key).ToString(), strTime, strAddr);
            } else {
