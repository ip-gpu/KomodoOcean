...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CKomodoAddress(CScriptID(redeemScript)).ToString();
                       ...
                       ZCIncrementalMerkleTree tree, bool added)
{
    ...
    } else LogPrintf("would have decremented %s nWitnessCacheSize.%d\n",ASSETCHAINS_SYMBOL,(int32_t)nWitnessCacheSize);
}
...
{
    //LogPrintf("A increment witness cache -> %d\n",(int32_t)nWitnessCacheSize);
    {
        LOCK(cs_wallet);
        for (std::pair<const uint256, CWalletTx>& wtxItem : mapWallet) {
            for (mapNoteData_t::value_type& item : wtxItem.second.mapNoteData) {
        ...
        if (nWitnessCacheSize < WITNESS_CACHE_SIZE) {
            //LogPrintf("increment nWitnesscache -> %d\n",(int32_t)nWitnessCacheSize);
            nWitnessCacheSize += 1;
        }
...
void CWallet::DecrementNoteWitnesses(const CBlockIndex* pindex)
{
    extern int32_t KOMODO_REWIND;
    {
        ...
        //LogPrintf("decrement witness cache -> %d\n",(int32_t)nWitnessCacheSize);
        if ( nWitnessCacheSize > 1 )
            nWitnessCacheSize -= 1;
        else
        {
            LogPrintf("%s nWitnessCacheSize.%d\n",ASSETCHAINS_SYMBOL,(int32_t)nWitnessCacheSize);
        }
        for (std::pair<const uint256, CWalletTx>& wtxItem : mapWallet) {
            for (mapNoteData_t::value_type& item : wtxItem.second.mapNoteData) {
                CNoteData* nd = &(item.second);
...
LogPrintf("wallet change %.8f (%.8f - %.8f) interest %.8f interest2 %.8f total %.8f\n",(double)nChange/COIN,(double)nValueIn/COIN,(double)nValue/COIN,(double)interest2/COIN,(double)interest/COIN,(double)nTotalValue/COIN);
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-komodo-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CKomodoAddress(address).ToString(), strPurpose))
        return false;
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CKomodoAddress(address).ToString();
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CKomodoAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CKomodoAddress(dest).ToString(), key);
}
