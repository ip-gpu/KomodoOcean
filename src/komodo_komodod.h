...
int32_t komodo_verifynotarizedscript(int32_t height,uint8_t *script,int32_t len,uint256 NOTARIZED_HASH)
{
    ...
    LogPrintf(" opreturn from [%s] ht.%d\n",ASSETCHAINS_SYMBOL,height);
    return(-1);
}
...
void komodo_index2pubkey33(uint8_t *pubkey33,CBlockIndex *pindex,int32_t height)
{
    ...
    {
        // height -> pubkey33
        //LogPrintf("extending chaintip komodo_index2pubkey33 height.%d need to get pubkey33\n",height);
    }
}
...
int32_t komodo_is_special(int32_t height,uint8_t pubkey33[33])
{
    ...
    if ( height >= 34000 && notaryid >= 0 )
    {
        ...
        for (i=1; i<limit; i++)
        {
            ...
            {
                if ( (0) && notaryid > 0 )
                    LogPrintf("ht.%d notaryid.%d already mined -i.%d nid.%d\n",height,notaryid,i,nid);
                if ( height > 225000 )
                    return(-1);
            }
        }
    ...
    *notarized_heightp = notarized_height;
    if ( notarized_height >= 0 && notarized_height <= pindex->nHeight && (notary= mapBlockIndex[notarized_hash]) != 0 )
    {
        ...
        if ( notary->nHeight == notarized_height ) // if notarized_hash not in chain, reorg
        {
            ...
            {
                LogPrintf("nHeight.%d < NOTARIZED_HEIGHT.%d\n",nHeight,notarized_height);
                return(-1);
            }
            else if ( nHeight == notarized_height && memcmp(&hash,&notarized_hash,sizeof(hash)) != 0 )
            {
                LogPrintf("nHeight.%d == NOTARIZED_HEIGHT.%d, diff hash\n",nHeight,notarized_height);
                return(-1);
            }
        } else LogPrintf("unexpected error notary_hash %s ht.%d at ht.%d\n",notarized_hash.ToString().c_str(),notarized_height,notary->nHeight);
