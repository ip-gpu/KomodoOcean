...
bool CheckProofOfWork(int32_t height,uint8_t *pubkey33,uint256 hash, unsigned int nBits, const Consensus::Params& params)
{
    ...
    if ( UintToArith256(hash) > bnTarget )
    {
            ...
            //&&  )//186269, 182507&& komodo_chainactive(height) != 0 && nonzpkeys > 0
        {
            ...
            LogPrintf(" ht.%d special.%d notaryid.%d ht.%d mod.%d error\n",height,special,notaryid,height,(height % 35));
            for (i=0; i<33; i++)
                LogPrintf("%02x",pubkey33[i]);
            LogPrintf(" <- pubkey\n");
            for (i=0; i<66; i++)
                LogPrintf("%d ",mids[i]);
            LogPrintf(" minerids from ht.%d\n",height);
            if ( KOMODO_REWIND == 0 && (notaryid >= 0 || height > 225000) )
            {
                LogPrintf("pow error height.%d loading.%d notaryid.%d\n",height,KOMODO_LOADINGBLOCKS,notaryid);
                return error("CheckProofOfWork(): hash doesn't match nBits");
            } else LogPrintf("skip return error height.%d loading.%d\n",height,KOMODO_LOADINGBLOCKS);
        } //else LogPrintf("skip height.%d loading.%d\n",height,KOMODO_LOADINGBLOCKS);
    }
    if ( 0 && height > 248000 )
    {
        for (i=31; i>=0; i--)
        ...
        for (i=31; i>=0; i--)
            LogPrintf("%02x",((uint8_t *)&bnTarget)[i]);
        LogPrintf(" POW ok for ht.%d notaryid.%d: ",height,notaryid);
        for (i=0; i<33; i++)
            LogPrintf("%02x",pubkey33[i]);
        LogPrintf("\n");
    }
