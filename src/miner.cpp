...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
...

    {
        ...
        if ( ASSETCHAINS_SYMBOL[0] != 0 && ASSETCHAINS_STAKED != 0 && NOTARY_PUBKEY33[0] != 0 )
        {
            ...
            } else LogPrintf("no utxos eligible for staking\n");
        }
        ...
        if ( !TestBlockValidity(state, *pblock, pindexPrev, false, false))
        {
            ...
            //if ( counter++ < 100 && ASSETCHAINS_STAKED == 0 )
                //LogPrintf("warning: miner testblockvalidity failed\n");
            return(0);
        }
    }
                ...
                std::function<bool(std::vector<unsigned char>)> validBlock =
                    ...
                    if ( ASSETCHAINS_STAKED == 0 )
                    {
                        ...
                        else if ( ASSETCHAINS_SYMBOL[0] != 0 )
                        {
                            ...
                            sleep(rand() % 30);
#else
                            boost::this_thread::sleep(boost::posix_time::milliseconds((rand() % 30)*1000));
#endif
