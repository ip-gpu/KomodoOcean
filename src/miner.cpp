...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    if ( ASSETCHAINS_SYMBOL[0] != 0 && komodo_baseid(ASSETCHAINS_SYMBOL) >= 0 && chainActive.Tip()->nHeight >= ASSETCHAINS_MINHEIGHT )
    {
        ...
        while ( KOMODO_ON_DEMAND == 0 && deposits == 0 && (int32_t)mempool.GetTotalTxSize() == 0 )
        {
            ...
            {
                //LogPrintf("INITDONE.%d RT.%d deposits %.8f ht.%d\n",KOMODO_INITDONE,isrealtime,(double)deposits/COIN,kmdheight);
            }
            else if ( komodo_isrealtime(&kmdheight) != 0 && (deposits != 0 || (int32_t)mempool.GetTotalTxSize() > 0) )
            {
                LogPrintf("start CreateNewBlock %s initdone.%d deposit %.8f mempool.%d RT.%u KOMODO_ON_DEMAND.%d\n",ASSETCHAINS_SYMBOL,KOMODO_INITDONE,(double)komodo_paxtotal()/COIN,(int32_t)mempool.GetTotalTxSize(),isrealtime,KOMODO_ON_DEMAND);
                break;
            }
#ifndef WIN32
            sleep(10);
        ...
        KOMODO_ON_DEMAND = 0;
        if ( 0 && deposits != 0 )
            LogPrintf("miner KOMODO_DEPOSIT %llu pblock->nHeight %d mempool.GetTotalTxSize(%d)\n",(long long)komodo_paxtotal(),(int32_t)chainActive.Tip()->nHeight,(int32_t)mempool.GetTotalTxSize());
    }
...

    {
             ...
             mi != mempool.mapTx.end(); ++mi)
        {
            ...
            {
                LogPrintf("CreateNewBlock: komodo_validate_interest failure\n");
                continue;
            }
        ...
        if ( !TestBlockValidity(state, *pblock, pindexPrev, false, false))
        {
            ...
            if ( counter++ < 100 )
                LogPrintf("warning: testblockvalidity failed\n");
            return(0);
        }
    }
...
CBlockTemplate* CreateNewBlockWithKey(CReserveKey& reservekey)
{
    ...
    {
        for (i=0; i<65; i++)
            LogPrintf("%d ",komodo_minerid(chainActive.Tip()->nHeight-i,0));
        LogPrintf(" minerids.special %d from ht.%d\n",komodo_is_special(chainActive.Tip()->nHeight+1,NOTARY_PUBKEY33),chainActive.Tip()->nHeight);
    }
    return CreateNewBlock(scriptPubKey);
}
...
#ifdef ENABLE_WALLET
void static KomodoMiner(CWallet *pwallet)
#else
void static KomodoMiner()
#endif
{
    ...
    try {
        ...
        while (true)
        {
                    ...
                    if ( nonzpkeys > 0 )
                    {
                        ...
                        if ( NOTARY_PUBKEY33[0] != 0 && (notaryid < 3 || notaryid == 34 || notaryid == 51 || notaryid == 52) )
                        {
                            ...
                            if ( externalflag == 0 && i != 66 )
                                LogPrintf("VIOLATION at %d\n",i);
                            for (i=0; i<66; i++)
                            { break;
                                for (j=0; j<33; j++)
                                    LogPrintf("%02x",pubkeys[i][j]);
            ...
            while (true)
            {
                ...
                {
                    LogPrintf("formerly valid mining block became invalid, likely due to tx expiration\n");
                    break;
                }*/
            }
        }
...
#ifdef ENABLE_WALLET
        minerThreads->create_thread(boost::bind(&KomodoMiner, pwallet));
#else
        minerThreads->create_thread(&KomodoMiner);
#endif
    }
}
